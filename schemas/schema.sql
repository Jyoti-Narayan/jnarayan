...
-- Page Views table for site analytics (stores comprehensive visitor data)
...
CREATE POLICY "Allow anonymous to insert page views" ON public.page_views
  FOR INSERT TO anon WITH CHECK (true);

CREATE POLICY "Allow anonymous to read page views" ON public.page_views
  FOR SELECT TO anon USING (true);

CREATE POLICY "Allow authenticated full access to page views" ON public.page_views
  FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Create indexes for faster queries
CREATE INDEX idx_page_views_created_at ON public.page_views(created_at);
CREATE INDEX idx_page_views_visitor_id ON public.page_views(visitor_id);
CREATE INDEX idx_page_views_is_new_visitor ON public.page_views(is_new_visitor);
CREATE INDEX idx_page_views_device_type ON public.page_views(device_type);
CREATE INDEX idx_page_views_browser ON public.page_views(browser);
